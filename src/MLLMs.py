...
class ModelWrapper(ABC):
    ...
    def get_spkr_prompt(self, intro, t, context_imgs, target_fn, interaction_args, records=[]):
    ...
    def get_lsnr_prompt(self, intro, t, context_imgs, target_fn, msg, records=[], random_seed=None, no_history=False, do_shuffle=False, omit_img=False, misleading=False, has_intro=True):
        if no_history:
            history=[]
  
        else:
            history=[entry['lsnr_trial_record'] for entry in records[:t]] if t>0 else []
        ...
        prompt=self._model_specific_prompt_postprocessing(prompt)
        return prompt, trial_prompt, target_img, target_label, trial_imgs_after_1st_shuffle, trial_imgs
    ...
    def get_lsnr_feedback(self, pred, target_img, context_imgs, spkr_msg):
        target_label=self.model_args.label_space[[img['filename'] for img in context_imgs].index(target_img['filename'])]
...
class IDEFICSModel(ModelWrapper):
    def __init__(self, model_args, loaded_model=None):
        ...
        if checkpoint=="HuggingFaceM4/idefics-9b-instruct":
            ...
            self.exit_condition = self.processor.tokenizer("<end_of_utterance>", add_special_tokens=False).input_ids
            self.bad_words_ids = self.processor.tokenizer(["<image>", "<fake_token_around_image>"], add_special_tokens=False).input_ids
        else:            
            quantization_config = BitsAndBytesConfig(load_in_8bit=True, bnb_8bit_compute_dtype="float16")
            self.model = IdeficsForVisionText2Text.from_pretrained(checkpoint, quantization_config=quantization_config, device_map="auto")
            self.processor = AutoProcessor.from_pretrained(checkpoint)
            self.exit_condition = self.processor.tokenizer("<end_of_utterance>", add_special_tokens=False).input_ids
            self.bad_words_ids = self.processor.tokenizer(["<image>", "<fake_token_around_image>"], add_special_tokens=False).input_ids
    ...
    def query(self, query):
        ...
        generation_output = self.model.generate(**inputs, eos_token_id=self.exit_condition, bad_words_ids=self.bad_words_ids, max_new_tokens=self.model_args.max_output_tokens, return_dict_in_generate=True, output_scores=True, do_sample=False)
...
class LlavaModel(ModelWrapper):
    ...
    def update_with_spkr_pred(self, spkr_trial_prompt, spkr_pred):
        spkr_trial_prompt[-1]=spkr_trial_prompt[-1]+' ASSISTANT: '+spkr_pred+'</s></s>'
        return spkr_trial_prompt
    
    def update_with_lsnr_pred(self, lsnr_trial_prompt, lsnr_pred):

        lsnr_trial_prompt[-1]=lsnr_trial_prompt[-1]+' ASSISTANT: '+lsnr_pred+'</s></s>'
        return lsnr_trial_prompt
...
class GPTModel(ModelWrapper): 
    def __init__(self, model_args, organization_ID, API_key, client=None):
        ...
        self.client = OpenAI(api_key=openai.api_key, organization=openai.organization, timeout=httpx.Timeout(15.0, read=5.0, write=10.0, connect=3.0))
        self.spkr_system_msg={"role": "system", "content": 'you are a smart assistant that will follow the user\'s instructions'} 
        self.lsnr_system_msg={"role": "system", "content": 'You are an assistant who will play a series of reference games with the user. You will pay close attention to the conversation history as more rounds are played.'}
        self.enforce_alternating_roles=False
        self.retry_after_seconds=15
        self.retry_limit=10
        assert model_args.img_mode=='URL', 'change the interaction template to support other input image types'
    ...
    def get_spkr_intro(self, context_imgs):
        ...
        "content": [
            ...
            {"type": "text", "text": f"\nImage {targetnames[0]}: "},
            {"type": "image_url", "image_url":{"url":context_imgs[0]['URL'], "detail": "low"}},
            {"type": "text", "text": f"\nImage {targetnames[1]}: "},
            {"type": "image_url", "image_url":{"url":context_imgs[1]['URL'], "detail": "low"}},
            {"type": "text", "text": f"\nImage {targetnames[2]}: "},
            {"type": "image_url", "image_url":{"url":context_imgs[2]['URL'], "detail": "low"}},
            {"type": "text", "text": f"\nImage {targetnames[3]}: "},
            {"type": "image_url", "image_url":{"url":context_imgs[3]['URL'], "detail": "low"}},
            ]
        } ]  
    ...
    def get_lsnr_intro(self):
        return [{
            "role":"user", 
            "content": [
              {"type": "text", "text": self.model_args.intro_text}
              ]
          } ]   
            ...
            prompt=[
                {
                    ...
                    {"type": "text", "text": f"{round_name}, "},
                    {"type": "text", "text": f"\nImage {label_space[0]}: "},
                    {"type": "image_url", "image_url":{"url":trial_imgs[0]['URL'], "detail": "low"}},
                    {"type": "text", "text": f"\nImage {label_space[1]}: "},
                    {"type": "image_url", "image_url":{"url":trial_imgs[1]['URL'], "detail": "low"}},
                    {"type": "text", "text": f"\nImage {label_space[2]}: "},
                    {"type": "image_url", "image_url":{"url":trial_imgs[2]['URL'], "detail": "low"}},
                    {"type": "text", "text": f"\nImage {label_space[3]}: "},
                    {"type": "image_url", "image_url":{"url":trial_imgs[3]['URL'], "detail": "low"}},
                    {"type": "text", "text": f"\nWhich image is this message referring to: {msg}? Output the image label only (a single letter)."},
                    ]
                }
            ]
    ...
    def _gpt_query(self, query, times_retried=0):
        ...
        system_msg=self.spkr_system_msg if self.model_args.role=='spkr' else self.lsnr_system_msg
        ...
        try:
            ...
            print(f"Rate limit exceeded. Waiting and retrying in {self.retry_after_seconds} seconds...")
            time.sleep(self.retry_after_seconds)
            times_retried+=1
            return self._gpt_query(query, times_retried=times_retried)
    ...
    def _get_spkr_feedback(self, pred_fn, spkr_tgt_img, spkr_trial_imgs):
        if pred_fn=='invalid':
            feedback={"role":"user", "content": [{"type": "text", "text": f"The listener didn't give a valid answer."}]}
        else:
            for i in range(4):
                if spkr_trial_imgs[i]['filename']==pred_fn:
                    pred_label=self.model_args.label_space[i]
            ...
            if pred_fn==spkr_tgt_img['filename']:
                feedback={"role":"user", "content": [{"type": "text", "text": f"The listener correctly answered Image {pred_label}."}]}
            else:
                feedback={"role":"user", "content": [{"type": "text", "text": f"The listener mistakenly answered Image {pred_label}."}]}
    ...
    def _get_lsnr_feedback(self, pred, target_label, spkr_msg=None):
            if pred==target_label:
                feedback={"role":"user", "content": [{"type": "text", "text": f"Correct."}]} 
            elif pred not in self.model_args.label_space:
                feedback={"role":"user", "content": [{"type": "text", "text": f"Invalid answer. Answer must be one of {self.model_args.label_space}."}]}
            else:
                feedback={"role":"user", "content": [{"type": "text", "text": f"Wrong, I'm referring to Image {target_label}."}]}
        ...
        if self.enforce_alternating_roles:
            ...
            for i in range(1, len(prompt)):
                if prompt[i]['role']==formatted_prompt[-1]['role']:
                    ...
                    formatted_prompt[-1]['content']=formatted_prompt[-1]['content']+prompt[i]['content']
                else:
...
class GeminiModel(ModelWrapper):
    ...
    def _gemini_query(self, query, times_retried=0):
        ...
        try:
            response = self.model.generate_content(query, generation_config=self.gemini_config, safety_settings={
...
class ClaudeModel(ModelWrapper):
    ...
    def get_spkr_intro(self, context_imgs):
        ...
        intro=[{'role': 'user', 
                'content': [{'type': 'text', 'text': intro_text},
                            ...
                            {'type': 'image', 'source': {'type': 'base64', 'media_type': 'image/jpeg', 'data': context_imgs[0][mode]}},
                            {'type': 'text', 'text': f"Image {label_space[1]}:"},
                            {'type': 'image', 'source': {'type': 'base64', 'media_type': 'image/jpeg', 'data': context_imgs[1][mode]}},
                            {'type': 'text', 'text': f"Image {label_space[2]}:"},
                            {'type': 'image', 'source': {'type': 'base64', 'media_type': 'image/jpeg', 'data': context_imgs[2][mode]}},
                            {'type': 'text', 'text': f"Image {label_space[3]}:"},
                            {'type': 'image', 'source': {'type': 'base64', 'media_type': 'image/jpeg', 'data': context_imgs[3][mode]}},
                            ]
                }]
    ...
    def _get_spkr_prompt(self, round_name, target_label):
        prompt=[{'role': 'user', 
                'content': [
                    {'type': 'text', 'text': f"{round_name}, the target is Image {target_label}."}
                    ]
                }]
            ...
            prompt=[
                        {
                            ...
                            "content": [
                            {"type": "text", "text": f"{round_name}, "},
                            {"type": "text", "text": f"which image is this message referring to: {msg}?"},
                            ]
                        },
                        {
                            "role": "assistant",
            ...
            prompt=[
                {
                    ...
                    {"type": "text", "text": f"{round_name}, "},
                    {"type": "text", "text": f"Image {label_space[0]}:"},
                    {'type': 'image', 'source': {'type': 'base64', 'media_type': 'image/jpeg', 'data': trial_imgs[0][mode]}},
                    {"type": "text", "text": f"Image {label_space[1]}:"},
                    {'type': 'image', 'source': {'type': 'base64', 'media_type': 'image/jpeg', 'data': trial_imgs[1][mode]}},
                    {"type": "text", "text": f"Image {label_space[2]}:"},
                    {'type': 'image', 'source': {'type': 'base64', 'media_type': 'image/jpeg', 'data': trial_imgs[2][mode]}},
                    {"type": "text", "text": f"Image {label_space[3]}:"},
                    {'type': 'image', 'source': {'type': 'base64', 'media_type': 'image/jpeg', 'data': trial_imgs[3][mode]}},
                    {"type": "text", "text": f"which image is this message referring to: {msg}?"},
                    ]
                },
                {
    ...
    def update_with_spkr_pred(self, spkr_trial_prompt, spkr_pred):
        spkr_pred_formatted={"role":"assistant", "content": [{"type": "text", "text": spkr_pred}]}
        spkr_trial_prompt.append(spkr_pred_formatted)
        return spkr_trial_prompt
    
    def update_with_lsnr_pred(self, lsnr_trial_prompt, lsnr_pred):

        lsnr_trial_prompt[-1]['content']=lsnr_trial_prompt[-1]['content']+' '+lsnr_pred
        return lsnr_trial_prompt
    ...
    def _get_lsnr_feedback(self, pred, target_label, spkr_msg=None):
        if pred==target_label:
            feedback={"role":"user", "content": [{"type": "text", "text": f"Correct, I'm referring to Image {target_label}."}]}
        elif pred not in self.model_args.label_space:
            feedback={"role":"user", "content": [{"type": "text", "text": f"Invalid answer. Answer must be one of {self.model_args.label_space}."}]}
        else:
            feedback={"role":"user", "content": [{"type": "text", "text": f"Wrong, I'm referring to Image {target_label}."}]}
